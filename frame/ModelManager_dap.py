...
class ModelManager():
    ...
    def __get_loss(self, logits, label):
        loss = 0
        if self.config.loss_func == 'CE':
            # loss = self.loss_func(logits.view(-1, self.config.num_class), label.view(-1)) 修改的
    ...
    def __SL_train(self, train_dataloader, test_dataloader):
        ...
        for epoch in range(1, self.config.epoch + 1):
            ...
            for batch in train_dataloader:
                train_loss = 0
                data, label, type = batch
                logits, _, pred_task = self.model(data, type)
                for key, value in logits.items():
                    train_loss += self.__get_loss(value, label[(type == int(key)).nonzero(as_tuple=True)[0]])
                train_loss += self.__get_loss(pred_task, type.cuda())*lam
                self.optimizer.zero_grad()
                train_loss.backward()
                self.optimizer.step()
                step += 1
                ...
                if step % self.config.interval_log == 0:
                    ...
                    # corrects = 0
                    # for key, value in logits.items():
                    #     corrects += (torch.max(value, 1)[1] == label[(type == int(key)).nonzero(as_tuple=True)[0]]).sum()
    ...
    def __SL_test(self, dataloader):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                type = torch.tensor([0]*len(data)).cuda()
                logits_dict, representation, _ = self.model(data, type)
                logits = logits_dict['0']
